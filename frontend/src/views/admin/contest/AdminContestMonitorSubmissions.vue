<script setup lang="ts">
import { ref, computed } from 'vue'
import { RefreshCw } from 'lucide-vue-next'

const userOrTeams = ['Team_Alpha', 'PwnMaster', 'CryptoKing', 'WebNinja', 'MiscHunter', 'ReverseLab', 'FlagSeeker', 'Newbie_01', 'CTF_Fan', 'ByteDancer']
const challenges = ['Web_Header', 'pwn_stack', 'RSA_101', 'Misc_QR', 'web_container', 'Reverse_ELF', 'SQLi_01', 'XSS_Reflect']
const flagSubmissions = ref(
  Array.from({ length: 28 }, (_, i) => ({
    id: i + 1,
    time: `2025-03-15 ${String(12 + Math.floor(i / 10)).padStart(2, '0')}:${String((i * 2 + 15) % 60).padStart(2, '0')}:${String((i * 3) % 60).padStart(2, '0')}`,
    userOrTeam: userOrTeams[i % userOrTeams.length],
    challengeTitle: challenges[i % challenges.length],
    flag: i % 5 === 0 ? 'flag{wrong}' : 'flag{***}',
    result: (i % 5 === 0 ? 'wrong' : 'correct') as 'correct' | 'wrong',
  }))
)
const flagPageSize = 10
const flagPage = ref(1)
const flagTotalPages = computed(() => Math.max(1, Math.ceil(flagSubmissions.value.length / flagPageSize)))
const paginatedFlagSubmissions = computed(() => {
  const start = (flagPage.value - 1) * flagPageSize
  return flagSubmissions.value.slice(start, start + flagPageSize)
})

function refresh() {
  flagPage.value = 1
}
</script>

<template>
  <div class="rounded-xl border border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-900 shadow-sm overflow-hidden">
    <section class="p-6">
      <div class="mb-4 flex items-center justify-between">
        <h3 class="text-lg font-semibold text-slate-800 dark:text-slate-100">
          Flag 提交记录
        </h3>
        <button
          type="button"
          class="inline-flex items-center gap-2 rounded-lg border border-slate-200 bg-white px-4 py-2 text-sm font-medium text-slate-700 transition hover:bg-slate-50 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-200 dark:hover:bg-slate-700"
          @click="refresh"
        >
          <RefreshCw class="h-4 w-4" />
          刷新
        </button>
      </div>
      <div class="overflow-x-auto rounded-lg border border-slate-200 dark:border-slate-700">
        <table class="w-full min-w-[640px] text-sm">
          <thead>
            <tr class="border-b border-slate-200 bg-slate-50 dark:border-slate-700 dark:bg-slate-800/60">
              <th class="px-4 py-3 text-left font-bold text-slate-600 dark:text-slate-300">
                时间
              </th>
              <th class="px-4 py-3 text-left font-bold text-slate-600 dark:text-slate-300">
                用户 / 队伍
              </th>
              <th class="px-4 py-3 text-left font-bold text-slate-600 dark:text-slate-300">
                题目
              </th>
              <th class="px-4 py-3 text-left font-bold text-slate-600 dark:text-slate-300">
                Flag
              </th>
              <th class="px-4 py-3 text-left font-bold text-slate-600 dark:text-slate-300">
                结果
              </th>
            </tr>
          </thead>
          <tbody>
            <tr
              v-for="row in paginatedFlagSubmissions"
              :key="row.id"
              class="border-b border-slate-100 dark:border-slate-700/80 hover:bg-slate-50 dark:hover:bg-slate-800/50"
            >
              <td class="px-4 py-3 font-mono text-xs text-slate-600 dark:text-slate-400">
                {{ row.time }}
              </td>
              <td class="px-4 py-3 text-slate-700 dark:text-slate-200">
                {{ row.userOrTeam }}
              </td>
              <td class="px-4 py-3 text-slate-700 dark:text-slate-200">
                {{ row.challengeTitle }}
              </td>
              <td class="px-4 py-3 font-mono text-xs text-slate-500 dark:text-slate-400">
                {{ row.flag }}
              </td>
              <td class="px-4 py-3">
                <span
                  :class="row.result === 'correct'
                    ? 'rounded-full bg-green-100 px-2 py-0.5 text-xs font-medium text-green-700 dark:bg-green-900/30 dark:text-green-300'
                    : 'rounded-full bg-slate-100 px-2 py-0.5 text-xs font-medium text-slate-600 dark:bg-slate-700 dark:text-slate-300'"
                >
                  {{ row.result === 'correct' ? '正确' : '错误' }}
                </span>
              </td>
            </tr>
          </tbody>
        </table>
      </div>
      <div v-if="flagTotalPages > 1" class="mt-3 flex items-center justify-end gap-2 text-sm text-slate-500 dark:text-slate-400">
        <button
          type="button"
          class="rounded px-2 py-1 hover:bg-slate-100 dark:hover:bg-slate-800 disabled:opacity-50"
          :disabled="flagPage <= 1"
          @click="flagPage--"
        >
          上一页
        </button>
        <span>{{ flagPage }} / {{ flagTotalPages }}</span>
        <button
          type="button"
          class="rounded px-2 py-1 hover:bg-slate-100 dark:hover:bg-slate-800 disabled:opacity-50"
          :disabled="flagPage >= flagTotalPages"
          @click="flagPage++"
        >
          下一页
        </button>
      </div>
    </section>
  </div>
</template>
