...
// 我的队伍逻辑 (Mock 当前用户 ID 为 1)
const myTeam = computed(() => {
  return teamStore.teams.find((t) => t.members.some((m) => m.id === 1));
});
  ...
  <div class="min-h-[calc(100vh-64px)] space-y-4">
    ...
    <div class="flex gap-6">
      ...
      <div class="flex-1 space-y-4">
        <!-- 搜索和筛选 -->
        <div
          class="flex w-full flex-col gap-4 rounded-xl border border-slate-200/80 bg-white/80 p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900/70"
        >
          <div class="flex flex-col gap-4 sm:flex-row sm:items-center">
            ...
            <div class="flex items-center gap-2">
              <label class="text-sm font-medium text-slate-600 dark:text-slate-300"> 状态： </label>
              <select
          ...
          <!-- 状态颜色说明 -->
          <div
            class="mt-1 flex items-center gap-4 border-t border-slate-100 pt-3 text-[10px] font-bold uppercase tracking-widest text-slate-400 dark:border-slate-800"
          >
            <span class="mr-2 text-slate-500 dark:text-slate-400">Legend:</span>
      ...
      <div class="w-80 shrink-0 space-y-6">
        <!-- 我的队伍卡片 -->
        <div
          class="rounded-xl border border-slate-200/80 bg-white/80 p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900/70"
        >
          <div class="flex items-center justify-between mb-4">
            ...
            <button
              class="rounded-lg p-1 text-blue-600 transition-colors hover:bg-blue-50 dark:text-blue-400 dark:hover:bg-blue-900/20"
              title="管理队伍"
          ...
          <div v-if="myTeam" class="space-y-4">
            <div class="flex items-center gap-3">
              <img
                :src="myTeam.avatar"
                class="h-12 w-12 rounded-xl border-2 border-white object-cover shadow-sm dark:border-slate-800"
              />
              <div class="min-w-0">
                <p class="truncate font-bold text-slate-900 dark:text-slate-50">
                  {{ myTeam.name }}
                </p>
            ...
            <div class="flex -space-x-2 overflow-hidden">
              <img
                v-for="member in myTeam.members.slice(0, 5)"
                :key="member.id"
                :src="member.avatar || `https://api.dicebear.com/7.x/avataaars/svg?seed=${member.username}`"
                class="inline-block h-7 w-7 rounded-full object-cover ring-2 ring-white dark:ring-slate-900"
                :title="member.username"
              />
              <div
                v-if="myTeam.members.length > 5"
                class="flex h-7 w-7 items-center justify-center rounded-full bg-slate-100 ring-2 ring-white dark:bg-slate-800 dark:ring-slate-900"
              >
                <span class="text-[10px] font-bold text-slate-500">+{{ myTeam.members.length - 5 }}</span>
              </div>
            </div>

            <button
              class="w-full rounded-lg border border-slate-100 bg-slate-50 py-2 text-xs font-bold text-slate-600 transition-all hover:bg-blue-50 hover:text-blue-600 dark:border-slate-700 dark:bg-slate-800/50 dark:text-slate-300 dark:hover:bg-blue-900/30 dark:hover:text-blue-400"
              @click="goToTeamManage"
            >
              进入队伍管理中心
            </button>
          ...
          <div v-else class="space-y-3 py-6 text-center">
            <div
              class="mx-auto flex h-12 w-12 items-center justify-center rounded-full bg-slate-50 dark:bg-slate-800"
            >
              <Users class="text-slate-300" :size="24" />
            </div>
        ...
        <!-- 历史参赛 -->
        <div
          class="rounded-xl border border-slate-200/80 bg-white/80 p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900/70"
        >
