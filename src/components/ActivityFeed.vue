...
<template>
  <div class="col-span-3">
        ...
        >
          ...
          <div v-if="feed.tags" class="mb-3 flex flex-wrap gap-2">
            <span
              ...
              :key="tag"
              class="rounded-button bg-slate-100 px-2 py-0.5 text-[10px] font-bold text-slate-500 transition-colors dark:bg-slate-800 dark:text-slate-400"
            >{{ tag }}</span>
          </div>
    ...
    <button
      class="rounded-inner mt-12 w-full border border-slate-200 bg-white py-3 text-sm font-bold text-slate-500 shadow-sm transition-all hover:bg-slate-50 hover:text-blue-600 dark:border-slate-800 dark:bg-slate-900 dark:text-slate-400 dark:hover:bg-slate-800 dark:hover:text-blue-400"
    >
      加载更多动态
    </button>
  </div>
